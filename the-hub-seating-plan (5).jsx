...
export default function TheHubSeatingPlan() {
  ...
  const submit = async () => {
    ...
    const existing = (await storageGet("hub-submissions", true)) || [];
    const stored = await storageSet("hub-submissions", [...existing, sub], true);
    sub.stored = stored;
  ...
  const loadSubmissions = async () => {
    setLoadingSubs(true);
    const subs = (await storageGet("hub-submissions", true)) || [];
    setSubmissions(subs.slice().reverse());
  ...
  const deleteSubmission = async (id) => {
    ...
    const subs = (await storageGet("hub-submissions", true)) || [];
    await storageSet("hub-submissions", subs.filter((s) => s.id !== id), true);
    setDeleteId(null);
    loadSubmissions();
  };
  const tryCode = () => {
